...
v17.0 修复：
  1. 排版按钮重复点击崩溃：移除 deleteLater，改用 disconnect + 显式生命周期管理
  2. 画笔操作触发排版线程堆积崩溃：QTimer 300ms 防抖，避免每帧都启动新线程
  3. layout_engine paste RGBA 崩溃：所有 paste 统一处理模式
"""
...
class PaintablePreview(QLabel):
    ...
    def _draw_at(self, p1, p2=None):
        ...
        self._img = img
        self._render()
        # 注意：不在这里 emit painted，由 mouseReleaseEvent 统一触发一次
    def mousePressEvent(self, e):
    ...
    def mouseReleaseEvent(self, e):
        if e.button()==Qt.LeftButton:
            self._last_pos=None
            # 只在松开鼠标时发出一次 painted 信号，避免每帧触发排版线程
            if self._paint_mode and self._img:
                self.painted.emit()
...
# 裁剪画布
# ─────────────────────────────────────────────
HIT_NONE=0; HIT_TL=1; HIT_TR=2; HIT_BL=3; HIT_BR=4
HIT_T=5; HIT_B=6; HIT_L=7; HIT_R=8; HIT_MOVE=9
NEAR=12
...
class CropCanvas(QLabel):
    def __init__(self, pil_img):
        super().__init__(); self.setAlignment(Qt.AlignCenter)
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self._pil = pil_img; self._iw,self._ih = pil_img.size
    ...
    def _scale(self):
        w=max(self.width()-4,1); h=max(self.height()-4,1)
        s=min(w/self._iw, h/self._ih)
        ow=(self.width()-int(self._iw*s))//2; oh=(self.height()-int(self._ih*s))//2
        return s,ow,oh
    def paintEvent(self, e):
        super().paintEvent(e)
        s,ow,oh = self._scale()
        iw_s=int(self._iw*s); ih_s=int(self._ih*s)
        # 绘制原图
        t=self._pil.copy(); t.thumbnail((iw_s,ih_s),Image.LANCZOS)
        px=pil_to_qpixmap(t)
        p=QPainter(self); p.drawPixmap(ow,oh,px)
        # 暗化遮罩
        p.fillRect(ow,oh,iw_s,ih_s,QColor(0,0,0,80))
        # 裁剪框
        x0=ow+int(self._cx0*s); y0=oh+int(self._cy0*s)
        x1=ow+int(self._cx1*s); y1=oh+int(self._cy1*s)
        p.setCompositionMode(QPainter.CompositionMode_SourceOver)
        # 清亮区
        t2=self._pil.crop((int(self._cx0),int(self._cy0),int(self._cx1),int(self._cy1)))
        tw=max(1,x1-x0); th=max(1,y1-y0)
        t2=t2.resize((tw,th),Image.LANCZOS)
        p.drawPixmap(x0,y0,pil_to_qpixmap(t2))
        # 边框
        pen=QPen(QColor(C['accent']),2); p.setPen(pen); p.drawRect(x0,y0,x1-x0,y1-y0)
        # 角点
    ...
    def _hit_test(self,mx,my):
        ...
        x0=ow+int(self._cx0*s); y0=oh+int(self._cy0*s)
        x1=ow+int(self._cx1*s); y1=oh+int(self._cy1*s)
        def near(a,b): return abs(a-b)<=NEAR
        def inr(v,a,b): return a-NEAR<=v<=b+NEAR
...
# ─────────────────────────────────────────────
# 后台线程（修复：不用 deleteLater，用 parent 管理生命周期）
# ─────────────────────────────────────────────
class RemoveBgWorker(QThread):
    done=pyqtSignal(object); fail=pyqtSignal(str)
    def __init__(self, img, parent=None):
        super().__init__(parent); self.img=img
    def run(self):
        try: self.done.emit(_rmbg_remove_background(self.img))
...
class LayoutWorker(QThread):
    ...
    def __init__(self, img, name, parent=None):
        super().__init__(parent); self.img=img; self.name=name
    def run(self):
...
# ─────────────────────────────────────────────
TEMPLATES = [
    ("一寸排版",    "25×35mm · 3×3 · 5寸纸"),
    ("二寸排版",    "35×49mm · 2×2 · 5寸纸"),
    ("小二寸排版",  "33×48mm · 2×2 · 5寸纸"),
    ("三寸排版",    "90×60mm · 1×2 · 5寸纸"),
    ("驾驶证排版",  "22×32mm · 5×2 · 5寸纸"),
    ("一寸+二寸排版","混排 · 7寸纸"),
    ("结婚照排版",  "50×35mm · 2×2 · 5寸纸"),
]
...
class App(QMainWindow):
    def __init__(self):
        ...
        # ── 修复1：worker 用 self 作为 parent，Qt 自动管理生命周期 ──
        self._rm_worker: RemoveBgWorker = None
        self._ly_worker: LayoutWorker = None
        # ── 修复2：画笔防抖定时器，松开鼠标后 200ms 才触发排版 ──
    ...
    def _build(self):
        ...
        self.info_lbl=StatusLabel(""); crop_row.addWidget(self.info_lbl)
        ll.addLayout(crop_row)
        ll.addWidget(divider())
        # STEP 2
        ll.addWidget(section_label("STEP 2  ·  AI 抠图"))
        ...
        self.rm_status=StatusLabel("上传照片后自动抠图"); ll.addWidget(self.rm_status)
        ll.addWidget(divider())
        # STEP 3
        ll.addWidget(section_label("STEP 3  ·  选择背景"))
        ...
        self._bg_btns=[]
        for i,(r,g,b,name) in enumerate(bg_colors):
            btn=BgColorBtn((r,g,b),name); btn.selected.connect(self._on_bg_color)
            if i==0: btn.set_active(True)
            self._bg_btns.append(btn); color_row.addWidget(btn)
        self._custom_btn=BgColorBtn((180,100,200),"自定义颜色",is_custom=True)
        ...
        self.bg_status=StatusLabel("当前背景：白色"); ll.addWidget(self.bg_status)
        ll.addWidget(divider())
        # STEP 4
        ll.addWidget(section_label("STEP 4  ·  选择排版"))
        self._btns={}
    ...
    def _stop_worker(self, w):
        """安全停止并断开 worker 的所有信号连接，避免 use-after-free"""
        if w is None:
            return None
        try:
            # 先断开所有信号，防止停止后仍触发槽函数
    ...
    def _start_remove_bg(self):
        ...
        # 修复：先 disconnect 旧 worker 的所有信号，再创建新 worker
    ...
    def _compose_and_show(self):
        ...
        self.preview_bg.set_bg_color(None if self._use_bg_img else self._bg_color)
    ...
    def _on_painted(self):
        """画笔松开时触发，用防抖定时器延迟更新排版，避免快速操作堆积线程"""
        img = self.preview_bg.result()
        if img:
            self._composed = img
            # 重启防抖定时器（200ms 内多次操作只触发一次排版）
